...
function lovelyMoon.joystickpressed(joystick, button)
	for index, state in pairs(_slotState.states) do
		if state and state._enabled and state.joystickpressed then
			state:joystickpressed(joystick, button)
		end
	end
end

function lovelyMoon.joystickreleased(joystick, button)
	for index, state in pairs(_slotState.states) do
		if state and state._enabled and state.joystickreleased then
